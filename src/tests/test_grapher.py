...
class Test(TestCase):
    def setUp(self) -> None:
        ...
        self.example_rxn_smiles = 'Br[Br:4].[CH2:0]1[NH:1][CH:2]1[CH3:3]>>[CH2:0]1[NH:1][CH:2]1[CH2:3][Br:4]'
        ...
        # graphs
        self.reactant_graph = build_mol_graph(self.reactants, dummy_atom_featurizer, dummy_bond_featurizer)
        self.product_graph = build_mol_graph(self.products, dummy_atom_featurizer, dummy_bond_featurizer)

    def test_get_atom_to_bond_maps(self):
        """The ground truth can be assessed with self.example_m.GetBondBetweenAtoms(x, y).GetIdx() """
        expected_a2b = {(0, 1): 0,  # C_sec -> N
                        (1, 2): 1,  # N -> C_tert
                        (2, 3): 2,  # C_tert -> methyl
                        (2, 0): 3,  # C_tert -> C_sec
                        }
    ...
    def test_create_cgr_reac_prod(self):
        ...
        with self.subTest("Testing atom order/features"):
            ...
            self.assertTrue(torch.all(torch.eq(torch.Tensor([[0., 0.],
                                                             [1., 4.],
                                                             [2., 0.],
                                                             [3., 1.],
                                                             [4., 2.],
        ...
        with self.subTest("Testing bond order/features"):
            self.assertTrue(torch.all(torch.eq(torch.Tensor([[1., 0.],  # CH2-NH bond
                                                             [1., 0.],
                                                             [2., 1.],  # CH-N bond
                                                             [2., 1.],
                                                             [3., 2.],  # acyclic C-C bond
                                                             [3., 2.],
                                                             [4., 4.],  # cyclic C-C bond
                                                             [4., 4.],
                                                             [0., 0.],  # lost bond Br-Br
                                                             [0., 0.],
                                                             [0., 3.],  # formed bond C-Br
                                                             [0., 3.]
                                                             ]), cgr.edata["e"])))
    ...
    def test_create_cgr_reac_diff(self):
        ...
        with self.subTest("Testing bond order/features"):
            self.assertTrue(torch.all(torch.eq(torch.Tensor([[1., -1.],  # CH2-NH bond
                                                             [1., -1.],
                                                             [2., -1.],  # CH-N bond
                                                             [2., -1.],
                                                             [3., -1.],  # acyclic C-C bond
                                                             [3., -1.],
                                                             [4., 0.],  # cyclic C-C bond
                                                             [4., 0.],
                                                             [0., 0.],  # lost bond Br-Br
    ...
    def test_create_cgr_prod_diff(self):
        ...
        with self.subTest("Testing bond order/features"):
            self.assertTrue(torch.all(torch.eq(torch.Tensor([[0., 1.],  # CH2-NH bond
                                                             [0., 1.],
                                                             [1., 1.],  # CH-N bond
                                                             [1., 1.],
                                                             [2., 1.],  # acyclic C-C bond
                                                             [2., 1.],
                                                             [4., 0.],  # cyclic C-C bond
                                                             [4., 0.],
                                                             [0., 0.],  # lost bond Br-Br
                                                             [0., 0.],
                                                             [3., -3.],  # formed bond C-Br
                                                             [3., -3.]
                                                             ]), cgr.edata["e"])))
    ...
    def test_atom_and_bond_mapping_returns_correct_ronly_atom(self):
        ...
        _, ronly_atom, _, _, _, _ = atom_and_bond_mapping(self.reactant_atom_map_numbers, self.product_atom_map_numbers,
                                                          self.reactant_a2b, self.product_a2b)
        self.assertEqual([0, ], ronly_atom)
    ...
    def test_atom_and_bond_mapping_returns_correct_ponly_atom(self):
        ...
        _, _, ponly_atom, _, _, _ = atom_and_bond_mapping(self.reactant_atom_map_numbers, self.product_atom_map_numbers,
                                                          self.reactant_a2b, self.product_a2b)
    ...
    def test_atom_and_bond_mapping_returns_correct_r2p_atom(self):
        ...
        """
        r2p_atom, _, _, _, _, _ = atom_and_bond_mapping(self.reactant_atom_map_numbers, self.product_atom_map_numbers,
                                                        self.reactant_a2b, self.product_a2b)
    ...
    def test_atom_and_bond_mapping_returns_correct_ronly_bond(self):
        ...
        """
        _, _, _, _, ronly_bond, _ = atom_and_bond_mapping(self.reactant_atom_map_numbers, self.product_atom_map_numbers,
                                                          self.reactant_a2b, self.product_a2b)
        self.assertEqual([0, ], ronly_bond)
    ...
    def test_atom_and_bond_mapping_returns_correct_ponly_bond(self):
        ...
        """
        _, _, _, _, _, ponly_bond = atom_and_bond_mapping(self.reactant_atom_map_numbers, self.product_atom_map_numbers,
                                                          self.reactant_a2b, self.product_a2b)
        self.assertEqual([3, ], ponly_bond)
    ...
    def test_atom_and_bond_mapping_returns_correct_r2p_bond(self):
        ...
        [(1, 0), (2, 1), (3, 2), (4, 4)]
        """
        _, _, _, r2p_bond, _, _ = atom_and_bond_mapping(self.reactant_atom_map_numbers, self.product_atom_map_numbers,
                                                        self.reactant_a2b, self.product_a2b)
